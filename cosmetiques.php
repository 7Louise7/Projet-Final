...
//page de présentation de la catégorie cosmétiques
?>
<!DOCTYPE html>
<html lang="fr">

<head>
	<title>Homemade - Cosmétiques</title>
	<meta charset="UTF-8">
...
//s'il a un abonnement standard, on affiche que les tutos standards qui ont comme thème cosmétiques	
		if ($idabonnement['idabonnement'] == 1) {
			$req = $bdd->query("SELECT tuto.idtuto, titreTuto, chemin FROM tuto, photo where theme='cosmetique' AND tuto.idtuto=photo.idtuto and tuto.idabonnement=1");
			$compteur = 1;
...
//sinon, on affiche tous les tutos qui ont comme thème cosmétiques  (comme il a un abonnement premium)
		else {
			$req = $bdd->query("SELECT tuto.idtuto, titreTuto, chemin FROM tuto, photo where theme='cosmetique' AND tuto.idtuto=photo.idtuto");
			$compteur = 1;
			while ($tuto = $req->fetch()) {
